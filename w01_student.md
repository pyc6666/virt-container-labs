...
1. 理解虛擬化的核心職責，分得出 Type 1 跟 Type 2 Hypervisor 差在哪。
2. 用多個維度比對 VM 跟 Container 的差異，說得出這門課為什麼走「VM 內跑 Docker」這條路。
3. 知道 vCPU 時間分片、記憶體分配、磁碟 provisioning 各自怎麼運作。
4. 知道 snapshot 能救你到什麼程度、不能救什麼，親手做一次故障注入跟回復。
5. 親手在 VMware 裝好 Ubuntu VM + Docker，四層驗證跑完拿到通行證，留下可重現交付。
...
虛擬化（Virtualization）是在一台實體機上模擬出多個獨立運算環境的技術。講白了就是一台電腦假裝自己是好幾台。核心價值有兩個：

- **隔離（Isolation）：** 每個虛擬環境互不干擾。一個 VM 裡的操作不會影響另一個 VM，也不會破壞你的 Host OS。最壞的情況就是刪掉 VM 重來。
...
> **想一想**：如果你在 4 核心筆電上開兩台 VM 各設 4 vCPU，總共 8 vCPU 超過實體核心數，VM 會怎樣？為什麼？
...
**如果直接在 Host OS 跑 Docker 會怎樣？** Windows 需要 WSL2 或 Docker Desktop，版本與設定差異大，排錯成本高；macOS 的 Docker Desktop 跑在隱藏 VM 裡（LinuxKit），行為與原生 Linux Docker 不完全一致。先統一到 VM 裡的 Ubuntu 再裝 Docker，是性價比最高的做法。
...
**過度配置（Overcommit）的問題：** 如果你在 4 核心的筆電上開兩台 VM 各設 4 vCPU（共 8 vCPU），就超過了實體核心數。Hypervisor 必須更頻繁地切換，每個 VM 拿到的實際 CPU 時間變少，效能下降、延遲增加。建議 vCPU 總數不超過實體核心的 50-75%，4 核心筆電開 1 台 VM 設 2 vCPU 即可。
...
> **想一想**：Ballooning 跟 Swapping 都是記憶體不夠時的應對手段，但效能差很多。你覺得兩者的差異在哪？為什麼 Swapping 是「最後手段」？
...
Snapshot 凍結的是某一時刻的**磁碟狀態**（靠差異磁碟）和可選的**記憶體狀態**（寫入 `.vmem` 檔）。它不是把整台 VM 複製一份——它只記錄「從這個時間點開始的變更」。
...
你可能會想：「snapshot 不就是備份嗎？」但其實差很多——snapshot 跟 VM 存在同一顆磁碟上，磁碟壞了就一起沒了。它只保護你「改壞設定」，不保護你「硬碟掛掉」。

Snapshot 的邊界（不保護的東西）：不保護 Host 端的檔案、不跨 VM 同步、不保護 VM 被刪除的情況。

#### 回復策略三要素

**命名規範：** snapshot 名稱要帶語意，例如 `clean-baseline`（OS 健康 + Docker 安裝前）、`docker-ready`（Docker 驗證通過）。像 `snap1`、`snap2` 這種名稱，時間久了完全無法判斷能不能安全回復。

**保留上限：** 建議最多 3 個活躍 snapshot，差異磁碟鏈過長會拖慢整台 VM。

**回復決策門檻：** 排錯 10 分鐘內找到根因就繼續修；超過 10 分鐘且根因不明，回復 snapshot 再重做。回復後必須重跑同一組驗證命令，不能只說「感覺正常」。
...
Ubuntu 使用 APT（Advanced Package Tool）管理套件。兩個最常用的命令必須按順序跑：

- **`apt update`**：更新「套件索引」（像是目錄），讓系統知道最新有哪些套件、在哪裡下載。這個命令**不安裝任何東西**。
- **`apt upgrade`**：根據索引，把已安裝的套件升級到最新版。

這就像你去超市之前要先看最新的 DM——沒看 DM 就直接去買，可能買到下架的商品。如果跳過 `apt update` 直接 `apt upgrade`，系統會根據過期索引去找套件，可能下載到舊版、找不到新版、甚至版本衝突。這是本課後續每週安裝新工具時的通用原則。
...
> 每一層失敗都會擋住後面的層。排錯時從上往下逐層檢查，不要跳到最底層重裝。
...
- alpine 通常只有幾 MB，nginx 數十 MB，hello-world 不到 50 KB——映像大小取決於包了多少東西。

#### 步驟 15：用 nginx 跑一個簡單 web server

- 這一步走完「拉映像 → 跑容器 → 對外服務」的完整流程。
- 命令：
...
- `-d` 代表背景執行（detach），`-p 8080:80` 把 VM 的 8080 port 對應到容器的 80 port。啟動後用 `docker logs` 可以看容器內程式的標準輸出。

#### 步驟 16：用 alpine 進入互動式容器

- 進入容器內部操作，觀察容器擁有獨立的檔案系統環境。
- 命令：
...
- 容器內的 hostname、OS、檔案系統都和你的 Ubuntu VM 不同，但共用 VM 的 kernel。

---

### Part A Checkpoint

> ✅ **Checkpoint A1**：VM 開得起來，資源跟 VMware 設定對得上
> - `whoami`、`hostnamectl`、`lscpu`、`free -h` 正常輸出，數值正確
> - 沒過？先查 BIOS 虛擬化有沒有開、VMware 硬體設定對不對

> ✅ **Checkpoint A2**：Docker 四層驗證全部通過，容器操作跑完
> - `hello-world` 成功、nginx 可 `curl`、alpine 可進入互動式 shell
> - 沒過？四層驗證逐層排查

---

### Part B：用 Snapshot 救自己一命
...
- 看到 delta vmdk 出現，就驗證了前面說的差異磁碟機制。
...
- 這次故障簡單，手動修復 30 秒就好。但如果改壞了多個設定檔又不確定改了什麼，手動修復成本會急劇上升——這時 snapshot 回復更划算。
...
> ✅ **Checkpoint B1**：兩個 snapshot 已建立，故障注入有三階段證據
> - `clean-baseline` 和 `docker-ready` 都出現在 Snapshot Manager
> - 有故障前、故障中、回復後的命令輸出對照
> - 沒過？回步驟 17 重新確認建點流程

> ✅ **Checkpoint B2**：snapshot 回復後完整驗證通過
> - `docker.list` 回來、`hello-world` 成功、nginx 可 `curl`、映像列表與回復前一致
> - 沒過？確認你回到的是 `docker-ready` 而不是 `clean-baseline`
...
踩坑很正常，下面是最常見的幾個，先看過一輪，遇到的時候會比較有方向：
...
這週的重點就一句話：環境能重建、能驗證、能回復，你才有資格說「我的環境沒問題」。下週見。
