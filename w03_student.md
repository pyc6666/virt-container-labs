...
1. 理解 Bastion → App → DB 分層架構的安全邏輯，講得出「最小暴露」是什麼意思。
2. 知道 SSH 金鑰認證怎麼運作，分得出它跟密碼差在哪。
3. 用 `ufw` 親手設好「先封後開」的防火牆。
4. 用 SSH ProxyJump 跳板連進內網 VM。
5. 親手破壞兩次（防火牆封鎖 + SSH 停掉），釐清 `timeout` 跟 `refused` 差在哪。
...
理解分層邏輯之後，來看本週要蓋的完整架構——包含每台 VM 的 IP 配置、防火牆規則和 SSH 連線路徑：
...
密碼可以被暴力破解（brute-force），攻擊者自動跑數百萬組密碼就有機會猜中。密碼也可能被側錄（keylogger）或社交工程騙走，而且很多人會在多處重複用同一組密碼——一處洩漏就全部失守。
...
這週你從「所有機器門戶大開」走到「分層管控、只留一個入口」。你親手設了金鑰認證（不再靠密碼這種脆弱的東西）、用 ufw 鎖好防火牆（先封後開，忘記開頂多服務不通，不會被偷打）、靠 ProxyJump 從外面跳進內網。最後你還故意破壞了兩次，親眼看到 `timeout` 跟 `refused` 長得不一樣、指向不同的問題。
